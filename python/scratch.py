...
# def _leveraged_df():
LEVERAGED_DF = pd.read_csv('leverage-symbol-mappings.csv')
...
def get_monthly_stats_df(start_date=START_DATE):
    dicts = [_get_monthly_stats_for_symbol(sym, start_date)
             for sym in all_relevant_symbols(dateCutoff=start_date)]
...
def _load_master_df(start_date=START_DATE):
    sym2closes = {}
    for symbol in all_relevant_symbols(start_date=start_date):
...
def main():
    ...
    # # # df = _load_monthly_history_for_symbol('DG')
    # # # # print(df['relMonth'].values[:10])
    # d = _get_monthly_stats_for_symbol('DG')
    # pprint(d)
    # print("DG" in all_relevant_symbols(dateCutoff=START_DATE))
    # return
    ...
    # sym = 'tmf'
    # df = _load_monthly_history_for_symbol(sym, start_date=None)
    # print("history df cols: ", df.columns.values)
    # print("leverage df cols: ", LEVERAGED_DF.columns.values)
    ...
    # print('drawdown stats:')
    # # print(mathutils.maxdrawdown(np.array([1, 2, 3, 4, 2, 5])))
    # # print(mathutils.maxdrawdown(np.array([1, 2, 3, 4, 3, 5])))
    # print(mathutils.maxdrawdown(np.array([0, 2, 3, 4, 3, 2, 1, 5])))
